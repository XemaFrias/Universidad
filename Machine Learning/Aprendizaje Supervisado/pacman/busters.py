...
def getNoisyDistance(gameState,pos1, pos2):
    if pos2[1] == 1: return None
    distanceCalc= Distancer(gameState.data.layout)
    distance = distanceCalc.getDistance(pos1, pos2)
...
class GameState:
    ...
    def generateSuccessor( self, agentIndex, action):
        ...
        state.data.ghostDistances = [getNoisyDistance(self,p, state.getGhostPosition(i)) for i in range(1,state.getNumAgents())]
    ...
    def getNearestFood(self):
        """
        Returns the x, y coordinates to the nearest food
        """
        if(self.getNumFood() > 0):
            minDistance = 900000
            pacmanPosition = self.getPacmanPosition()
            distanceCalc= Distancer(self.data.layout)
            for i in range(self.data.layout.width):
                for j in range(self.data.layout.height):
                    if self.hasFood(i, j):
                        foodPosition = i, j
                        distance = distanceCalc.getDistance(pacmanPosition, foodPosition)
    ...
    def initialize( self, layout, numGhostAgents=1000 ):
        ...
        self.data.ghostDistances = [getNoisyDistance(self,self.getPacmanPosition(), self.getGhostPosition(i)) for i in range(1, self.getNumAgents())]
        self.ghostPositions = [self.getGhostPosition(i) for i in range(1, self.getNumAgents())]

        if not os.path.isfile('basicAgentAA.arff'):
            f = open("basicAgentAA.arff","a")
            f.write("   @RELATION Pac-Man\n")
            f.write("   @ATTRIBUTE pacmanX NUMERIC\n")
            f.write("   @ATTRIBUTE pacmanY NUMERIC\n")
            f.write("   @ATTRIBUTE isNorthLegal {True, False}\n")
            f.write("   @ATTRIBUTE isSouthLegal {True, False}\n")
            f.write("   @ATTRIBUTE isEastLegal {True, False}\n")
            f.write("   @ATTRIBUTE isWestLegal {True, False}\n")
            f.write("   @ATTRIBUTE isStopLegal {True, False}\n")
            f.write("   @ATTRIBUTE blueGhostX NUMERIC\n")
            f.write("   @ATTRIBUTE blueGhostY NUMERIC\n")
            f.write("   @ATTRIBUTE orangeGhostX NUMERIC\n")
            f.write("   @ATTRIBUTE orangeGhostY NUMERIC\n")
            f.write("   @ATTRIBUTE cianGhostX NUMERIC\n")
            f.write("   @ATTRIBUTE cianGhostY NUMERIC\n")
            f.write("   @ATTRIBUTE yellowGhostX NUMERIC\n")
            f.write("   @ATTRIBUTE yellowGhostY NUMERIC\n")
            f.write("   @ATTRIBUTE livingGhosts {0,1,2,3,4}\n")
            f.write("   @ATTRIBUTE closestGhostDistance NUMERIC\n")
            f.write("   @ATTRIBUTE remainingFood NUMERIC\n")
            f.write("   @ATTRIBUTE closestFoodDistance NUMERIC\n")
            f.write("   @ATTRIBUTE nearestFoodX NUMERIC\n")
            f.write("   @ATTRIBUTE nearestFoodY NUMERIC\n")
            f.write("   @ATTRIBUTE class {North, South, East, West, Stop}\n")
            f.write("   @ATTRIBUTE Score NUMERIC\n")
            f.write("   @ATTRIBUTE Score1 NUMERIC\n")
            f.write("   @ATTRIBUTE Score2 NUMERIC\n")
            f.write("   @ATTRIBUTE Score3 NUMERIC\n")
